...
class CardDeckOperationsSpec extends FlatSpec {
  ...
  it should "return list in a different order when it has more than 2 elements" in {
    val result = List(Blank, Explosive, Blank)
    assert(result.shuffle !== result)
  }
